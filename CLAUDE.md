...
- A write whose body carries a secret (consumer create and credential writes, plugin configurations, upstreams, TLS key material and ACME account credentials, batch create) sends with `SILENT_ERRORS` and runs inside `withRedactedFailure(secretValues(body), …)` (`src/api/secretRedaction.ts`): its failure becomes a `RedactedWriteError` with every submitted secret replaced by `[REDACTED]` in the message and parsed body — redacted before any trim or truncation — plus a bodiless response for the status, and no `request`, `options`, or `cause`. Add any new secret-bearing write to it. See `docs/concurrent-edits.md`
