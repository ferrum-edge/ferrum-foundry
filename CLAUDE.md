...
the published Ferrum Edge v0.9.7 release, the release Foundry v0.2.0 pairs with
(`edge.release`). `edge.image` in `docs/compatibility.json` is the single source: CI
...
- `docs/compatibility.md` / `docs/compatibility.json` - the Foundry–Edge pairing: the one Edge image CI qualifies (`edge.image`, the published v0.9.7 release), the published Edge release the Foundry release pairs with (`edge.release`, v0.9.7, with its requirements: ferrum-edge#5661 plus the starter and critical journeys passing), Edge images evaluated and rejected, the pin history, the tested envelope, and what is best-effort or not qualified. `docs/release-notes/vX.Y.Z.md` is the notes for `foundry.version` (v0.2.0) and `docs/release-notes/UNRELEASED.md` drafts the next release's; the release workflow requires `docs/release-notes/vX.Y.Z.md`, a matching `foundry.version` and `package.json` version, and `supported-pairing.mjs release-ready`
...
- A write whose body carries a secret (consumer create and credential writes, plugin configurations, upstreams, TLS key material and ACME account credentials, batch create, backup restore via `restoreSecrets`, API spec import/replace via `specDocumentSecrets`) runs inside `withRedactedFailure(secretValues(body), …)` (`src/api/secretRedaction.ts`): its failure becomes a `RedactedWriteError` with every submitted secret replaced by `[REDACTED]` in the message and parsed body — redacted before any trim or truncation — plus a bodiless response for the status, and no `request`, `options`, or `cause`. Its request carries `REDACT_ERRORS` (`src/api/client.ts`), which holds the global popup's report until the wrapper raises it redacted — same status, URL, and outcome, so a lost answer still opens "Outcome unknown" — or `SILENT_ERRORS` when the form reports every failure itself; never neither, since the popup would show the raw body. A plugin `config` is classified by Ferrum Edge's own projection table (`PLUGIN_SENSITIVITY` in `src/api/pluginSensitivity.ts`, transcribed from `plugin_config_projection.rs` at `edge.source_commit`; `scripts/plugin-sensitivity-drift.mjs` diffs it against that source in the Pinned Gateway Contract job, and its test fails when `PLUGIN_SENSITIVITY_SOURCE.commit` is not the pinned commit). A plugin it does not name is secret throughout, as is every scalar of a YAML spec document Foundry cannot parse; such an *unclassified* value shorter than 8 characters is redacted only as a whole token of a string, so `a`, `on`, or `error` cannot wipe out a `[REDACTED]` marker. No value shorter than 8 characters, classified or not, is matched in an object key or in the fixed-vocabulary fields callers decide on (`code`, `phase`, `rollback`, `failure_class`, `confirmation_required`), so a short credential cannot hide a restore's confirmation or recovery details. `RedactedWriteError` keeps the original error's `name` for the outcome classifiers, and secret-bearing mutation hooks set `gcTime: 0`. Redaction replaces every match found in the original text in one pass. Add any new secret-bearing write to it. See `docs/concurrent-edits.md`
...
- The admin API's list endpoints accept **`offset` and `limit` only** — no search, filter, or reference query — with one exception: `GET /plugins/config?proxy_id=` (Edge v0.9.7, ferrum-edge#5726) lists the proxy-scoped configurations targeting one proxy. `plugins.listConfigsForProxy()` uses it and refuses a response the filter did not narrow; it cannot answer effective policy, which also needs global and proxy-group configurations. Ordinary navigation must still be bounded: a list page fetches one page, references are resolved for the rows on screen (`useUpstreamReferences`), expensive secondary views load when their tab is opened, and a *summary* that would need a whole-collection scan stops at `SUMMARY_SCAN_BUDGET` and reports unavailable rather than a smaller number. A conclusion that is wrong if partial — effective policy, membership — is complete or unknown, never partial. Traversals take the Query's `AbortSignal` so a namespace switch abandons them; membership plans deliberately pass none. See `docs/data-loading.md`
...
- **Every namespace-scoped API function takes a `NamespaceScope` as its first argument** (`src/api/client.ts`) and builds its ky options with `scoped(scope, …)`, which stamps `X-Ferrum-Namespace`. Hooks read `scope` from `useNamespace()` once per operation and pass it through every request the operation makes (all `listAll()` pages, membership preflight/apply/rollback via `bindPluginMembership(scope)`, restore, and the apply-status poll). The client never reads the namespace from storage; a gateway request without a binding that is not `FLEET_GLOBAL` is refused with `UnboundNamespaceError`
- `NamespaceProvider` (`src/stores/namespace.tsx`) owns the active namespace per tab. `localStorage` is a load-time preference only: it is read once at mount and written on switch, and cross-tab `storage` events are deliberately ignored so another tab's switch can never retarget a running operation. See `docs/authentication.md` → "Namespace binding"
...
- Detail-page editors are bound to `{ namespace, resourceId }` (`src/lib/editorIdentity.ts`, `useEditorIdentity` in `src/hooks/useEditorIdentity.ts`): the route component owns the session and renders the editor as `<Editor key={session.key} session={session} />`, so a namespace switch or route change remounts the form, credential drafts, inline target editors, and confirmation dialogs against the new resource instead of carrying the previous tenant's fields across a cached switch. Wrap submit/confirm handlers in `session.bind()` (call `preventDefault()` outside it) so a call that outlives its editor is discarded. Forms seed their state once per identity and never rewrite fields on a background refetch of the same identity; do not put `updated_at` or other per-response values in the key. Because of that seed-once rule a mutation must **retire** (`removeQueries`) the scoped detail key of every resource it deleted or re-created, not merely invalidate it; mutations whose gateway effect cascades across resource types (spec import/replace/delete, proxy delete, namespace restore) do so through `retireCascade()` in `src/hooks/retireCascade.ts`, by namespace prefix, using the namespace the mutation was issued under. See `docs/authentication.md` → "Editor identity"
