...
- Foundry is now qualified against an explicit, published Ferrum Edge release: **v0.9.5**, pinned by its multi-architecture digest `ferrumedge/ferrum-edge@sha256:eca46c84bca92d6ef467979f8846537f7ab56c0cdc137befff465526a10fe10f`. It replaces the previous pin, `ferrumedge/ferrum-edge@sha256:fb0f05b0392a272ba36a493584bced171655ce8ebd36b2ae0818bb5c3c25ef2d`, an unreleased Edge `main` build from 2026-08-27. `docs/compatibility.json` is the single source: CI reads the gateway image from it, and `scripts/supported-pairing.test.mjs` fails when the starter, the local-run instructions, the README, or any doc pins a different Edge image or still names a retired one (#385).
...
- Local gateway setup docs now run the same image digest as the `Pinned Gateway Contract` CI job, the e2e suite, and the deployment starter, so local checks exercise the gateway CI does. Agent instructions no longer ship the Rust gateway's `.claude/rules` (which loaded Cargo test and CI policy into Foundry sessions), and the agent-dispatch skills drop leftover gateway vocabulary, fixed update cadences, and the duplicate Fable 5 skill. Dispatched workers now validate locally (typecheck, lint, affected tests, build when relevant) before pushing, with CI confirming the pushed head, replacing the no-local-build rule inherited from the Rust gateway (#395).

### Added

- A versioned compatibility record (`docs/compatibility.md`, `docs/compatibility.json`) naming the supported Foundry–Edge pairing, the tested envelope (modes, roles, authentication, deployment path, browser, container platforms, measured scale), what is best-effort, what is not qualified, and the dependency on unreleased Edge work: atomic concurrent-edit protection needs ferrum-edge#5661, which no published Edge release includes yet. Release notes are drafted in `docs/release-notes/UNRELEASED.md`; the release workflow now requires `docs/release-notes/vX.Y.Z.md` and a matching `foundry.version` before building, and publishes those notes as the GitHub release body (#385).
